...
class Detector:
    ...
    def createBoundigBox(self, image, threshold):
        ...
        if len(bboxIdx) != 0:
            for i in bboxIdx:
                ...
                self.detections_list.append([classLabelText, classConfidence, xmin, xmax, ymin, ymax])
                ############################
    ...
    def predictImage(self, imagePath, outputPath, threshold=0.5):
        image = cv2.imread(imagePath)

        bboxImage = self.createBoundigBox(image, threshold)

        outputName = "prediction.png"
        os.makedirs(outputPath, exist_ok=True)

        cv2.imwrite(os.path.join(outputPath, outputName), bboxImage)
