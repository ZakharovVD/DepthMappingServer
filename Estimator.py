...
def run(dataset, option):
    ...
    # Generate mask used to smoothly blend the local path estimations to the base estimate.
    ...
    for image_ind, images in enumerate(dataset):
        ...
        # Computing a scale in case user prompted to generate the results as the same resolution of the input. Notice
        # that our method output resolution is independent of the input resolution and this parameter will only
...
def init_methods(depthNetName):
    print(f'Starting initialization {depthNetName} method')

    if depthNetName == "midas":
        midas_model_path = "midas/model.pt"
        global midasmodel
        midasmodel = MidasNet(midas_model_path, non_negative=True)
        midasmodel.to(device)
        midasmodel.eval()
        if midasmodel is not None:
            print("Successfully initialization midas")

    elif depthNetName == "leres":
        global leresmodel
        ...
        leresmodel.eval()
        if leresmodel is not None:
            print("Successfully initialization LeRes")
    else:
        assert False, str(f'Incorrect input, inspect {depthNetName} instead of midas/leres')


def setThreshold(whsThreshold, gpuThreshold):
    global whole_size_threshold, GPU_threshold
    whole_size_threshold = whsThreshold
    GPU_threshold = gpuThreshold
    getThreshold()


def getThreshold():
    print(f'whole_size_threshold:  {whole_size_threshold}')
    print(f'GPU_threshold:  {GPU_threshold}')
    return whole_size_threshold, GPU_threshold


def estimateImage(argv):
    ...
    option, _ = parser.parse_known_args(argv)

    if int(option.R0) + int(option.R20) + int(option.Final) == 0:
        assert False, 'Please activate one of the [R0, R20, Final] options using --[R0]'
    elif int(option.R0) + int(option.R20) + int(option.Final) > 1:
    ...
    if option.depthNet == 0:
        ...
        global leresmodel
        if leresmodel is None:
            init_methods("leres")

    dataset_ = ImageDataset(option.data_dir, 'test')
    run(dataset_, option)
